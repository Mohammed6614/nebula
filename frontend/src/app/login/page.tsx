  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: error.response?.data?.message || 'يرجى التحقق من بيانات الدخول',
        variant: 'destructive',
      });
      setIsLoading(false);
    }
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <Link href="/forgot-password" className="text-nebula-600 hover:text-nebula-700">
                  نسيت كلمة المرور؟
                </Link>
              </div>
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                ليس لديك حساب؟{' '}
                <Link href="/register" className="text-nebula-600 hover:text-nebula-700 font-medium">
                  إنشاء حساب جديد
                </Link>
              </p>
            </div>
